...
<head>
  ...
  <title>VidView | Video Reports</title>
  <link rel="shortcut icon" href="https://ui-avatars.com/api/?name=Reports&background=0ea5e9&color=fff&rounded=true" type="image/x-icon">
        ...
        <table class="w-full text-sm text-left">
          <thead class="text-xs uppercase bg-gray-800 text-gray-300">
            <tr>
              ...
              <th class="px-6 py-4">Reported At</th>
          ...
          <tbody id="table" class="text-gray-200">
            <c:choose>
              <c:when test="${not empty reports}">
                <c:forEach var="report" items="${reports}">
                  <tr class="border-b border-gray-800 hover:bg-gray-800/60 transition">
                    ...
                    <td class="px-6 py-4">${report.videoTitle}</td>
                    <td class="px-6 py-4">${report.reportedBy}</td>
                    <td class="px-6 py-4">${report.reason}</td>
                    <td class="px-6 py-4">${report.reportedAt}</td>
              ...
              <c:otherwise>
                <tr>
                  <td colspan="6" class="px-6 py-4 text-center text-gray-400">No reports available.</td>
                </tr>
              </c:otherwise>
            </c:choose>
          </tbody>
